...
bool SqlParser::is_valid(Command t_command)
{
    switch(t_command) {
...
void SqlParser::parse(Packet & query_packet, std::vector<unsigned char> query_data)
{
    for (unsigned char & byte : query_data)
    {
        switch (currentState) {
            case State::Payload_length_0: {
        ...
        if (query_packet.current_index_ >= query_packet.query_length_) {
            currentState = State::Payload_length_0;
